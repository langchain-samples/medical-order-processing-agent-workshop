...
same time. Any resource addressed by a shared *name* — tracing projects, hub
prompts, eval datasets, deployments, annotation queues, run rules — will be
...
def workshop_user() -> str:
    ...
    if slug != raw:
        # `.env` interpolates the *raw* value into LANGSMITH_PROJECT, while
        # resources created through this helper use the slug. Anything needing
        # normalization therefore splits the two apart.
        warnings.warn(
            f"WORKSHOP_USER={raw!r} was normalized to {slug!r}. Your "
            f"LANGSMITH_PROJECT still carries the raw value, so your tracing "
            f"project and your other resources will be named inconsistently. "
            f"Set WORKSHOP_USER=\"{slug}\" in .env to keep them aligned.",
            stacklevel=2,
        )
    return slug
